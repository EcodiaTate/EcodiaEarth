  ...
  return (
    <main className="relative min-h-screen w-full overflow-hidden bg-[#F9F8F5] font-mono text-[#2D2B28] selection:bg-[#7FD069]">
      ...
      <div className="fixed right-[8%] top-12 z-50">
        <SmoothLink href="/" className="group flex items-center gap-4 border-b border-[#2D2B28] pb-1 transition-all">
          <ArrowLeft size={14} className="group-hover:-translate-x-2 transition-transform duration-700" />
          <span className="text-[10px] font-black uppercase tracking-[0.4em]">Back to index</span>
        </SmoothLink>
      </div>
      ...
      <div className="relative z-10 pl-[8%] pr-[15%] py-32">
            ...
            >
              ...
              <div className="space-y-16 max-w-2xl border-l border-[#2D2B28] pl-12">
                <p className="text-3xl font-medium leading-tight tracking-tight">
                  The leaf is the smallest unit. A mark left after something real.
                </p>

                <div className="space-y-10">
                  <p className="text-[10px] font-black uppercase tracking-[0.5em] opacity-40 italic">Placement</p>
                  <ul className="space-y-12">
                    {[
                      { id: "01", text: "Where hands already go - corners, hinges, touch points." },
                      { id: "02", text: "Where wear is honest - surfaces that last." },
                      { id: "03", text: "As a maintenance note - proof that someone cared." },
                    ].map((item) => (
                      <li key={item.id} className="flex items-start gap-8 group">
                        <span className="text-xs font-black opacity-20 group-hover:opacity-100 transition-opacity">[{item.id}]</span>
                        <span className="text-xl opacity-80 leading-snug">{item.text}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="lg:col-span-8">
              <div className="space-y-12">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.6em] opacity-40">State marker</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-end">
                  <p className="text-2xl font-medium opacity-70 leading-tight">
                    Motion without closure. The moment before the mark is made.
                  </p>
          ...
          <div className="border-t-8 border-[#2D2B28] pt-24">
            <div className="grid grid-cols-1 gap-20 lg:grid-cols-12">
              <div className="lg:col-span-6">
                <div className="text-[10px] font-black uppercase tracking-[0.6em] mb-12 opacity-30">Constant</div>
                <h2 className="text-[14vw] font-black leading-none tracking-tighter lg:text-[18rem]">ECO</h2>
              </div>
              <div className="lg:col-span-6">
                <div className="space-y-20">
                  <p className="text-4xl font-medium leading-[1.1] tracking-tighter">
                    Energy of shared progress. Earned through work, not bought.
                  </p>
        ...
        <footer className="mt-64 flex flex-col items-start justify-between border-t border-[#2D2B28]/10 pt-16 md:flex-row opacity-30">
          <div className="text-[9px] uppercase tracking-[0.8em]">Field notes • 01</div>
          <div className="flex gap-1 my-8 md:my-0">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="h-px w-6 bg-[#2D2B28]" />
            ))}
          </div>
          <div className="text-[9px] uppercase tracking-[0.8em]">Registry • 2025</div>
        </footer>
      </div>
    </main>
  );
...
function MetricBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[#F9F8F5] p-12 group transition-all duration-700 hover:bg-[#2D2B28] hover:text-[#F9F8F5]">
      <div className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 mb-4 group-hover:opacity-100">{label}</div>
