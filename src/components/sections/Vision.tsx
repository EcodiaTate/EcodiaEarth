...
/**
 * ECODIA VISION
 * Materiality: Grainy dark → warm paper
 * Strategy: Document the shift, without explaining it to death.
 */
  ...
  return (
    <main
      className={`relative w-full min-h-screen transition-colors duration-1000 ${
        mode === "system" ? "bg-[#F9F8F5] text-[#2D2B28]" : "bg-[#080808] text-white"
      }`}
    >
      {/* Exit */}
      <motion.button
      ...
      <div className="relative z-30 -mt-[100vh]">
        <Scene
          ...
          align="left"
          kicker="Log 01 // Drag"
          title="IT FELT LIKE DRAG."
          body="Good intent stayed private. Participation cost time, attention, energy."
          notes={["Drag", "Private effort", "Drop-off"]}
        />
        ...
        <Scene
          ...
          align="right"
          kicker="Log 02 // Defaults"
          title="BETTER DEFAULTS."
          body="Ecodia makes participation normal. Clear prompts. Repeatable conditions. You step in."
          notes={["Entry", "Shared upside", "Momentum"]}
        />
        ...
        <SceneCenter
          ...
          kicker="Protocol active"
          title="A WORLD IN USE."
          body="Action leaves marks. Those marks change the setting. The rest follows."
          columns={[
            {
              title: "Seen",
              items: ["Progress shows up in place", "Care sits in structure", "Momentum moves through places"],
            },
            {
              title: "Return",
              items: ["Personal and shared overlap", "Local economies keep the gain", "Value follows participation"],
            },
            {
              title: "Carry",
              items: ["Work outlives attention", "Communities keep memory", "The grid keeps building"],
            },
          ]}
        />
    ...
    <div className="relative w-full h-full overflow-hidden">
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-1000 ${
          mode === "system" ? "opacity-20 grayscale" : "opacity-40"
        }`}
      >
        <Image
      ...
      <div
        className={`absolute inset-0 bg-[#080808] transition-opacity duration-1000 ${
          mode === "friction" ? "opacity-80" : "opacity-0"
        }`}
      >
      ...
      {/* System: warm paper / grid */}
      <div
        className={`absolute inset-0 bg-[#F9F8F5] transition-opacity duration-1000 ${
      ...
      >
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(#2D2B28 1px, transparent 1px), linear-gradient(90deg, #2D2B28 1px, transparent 1px)",
...
function InertialHUD({ mode }: { mode: ViewMode }) {
  return (
    <div className="absolute inset-0 pointer-events-none z-20 p-12 flex flex-col justify-between">
      ...
      <div
        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-40 w-40 border border-current transition-all duration-1000 ${
          mode === "system" ? "opacity-10 scale-100" : "opacity-0 scale-150"
        }`}
      />
        ...
        <div
          className={`${
            align === "left"
              ...
              : "col-span-12 lg:col-start-7 lg:col-span-6"
          } space-y-12`}
        >
